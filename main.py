...
async def _auto_assign_loop():
    ...
    while True:
        ...
        # ── Simulate authority work progression ──────────────────
        # Work items advance new → acknowledged → in_progress → completed
        # on staggered timers (faster for higher severity), so the kanban
        # board shows realistic movement without manual clicks.
        _PROGRESSION = {
            "new":          ("acknowledged", "👀 {label} acknowledged"),
            "acknowledged": ("in_progress",  "🔧 {label} started work on-site"),
            "in_progress":  ("completed",    "✅ {label} marked resolved"),
        }
        _SEV_SPEED = {"critical": 0.5, "high": 0.7, "medium": 1.0, "low": 1.4}

        for (event_id, auth), wi in list(_work_items.items()):
            nxt = _PROGRESSION.get(wi["status"])
            if not nxt:
                continue
            ev = engine.events.get(event_id)
            if not ev:
                continue
            # Stagger: stable per-item base delay (12–40 s) scaled by severity
            seed  = hash((event_id, auth, wi["status"])) % 29
            base  = 12 + seed                     # 12–40 s
            delay = base * _SEV_SPEED.get(ev.severity, 1.0)
            if now - wi["ts_updated"] < delay:
                continue
            new_status, msg_tpl = nxt
            wi["status"]     = new_status
            wi["ts_updated"] = now
            ev.timeline.append({
                "ts":  now,
                "msg": msg_tpl.format(label=_AUTHORITY_META[auth]["label"]),
            })
...
# ════════════════════════════════════════════════════════════
# AI Copilot — server-side Groq proxy
# The Groq key stays on the server (read from env or a gitignored .env);
# the browser only ever talks to /api/copilot, so the key is never shipped
# in the client bundle or visible in network requests.
# ════════════════════════════════════════════════════════════
...
def _groq_key() -> str:
    """Resolve the Groq key server-side: GROQ_API_KEY env → .env files. Never exposed."""
    ...
    for env_path in (Path(__file__).parent / ".env",
        ...
        try:
            for line in env_path.read_text().splitlines():
                line = line.strip()
                if line.startswith(("GROQ_API_KEY", "VITE_GROQ_API_KEY")) and "=" in line:
...
def _groq_chat(messages: list, api_key: str) -> str:
    """Blocking Groq chat call (run via asyncio.to_thread)."""
    body = json.dumps({
        "model":       GROQ_MODEL,
        "messages":    messages,
        "temperature": 0.6,
    ...
    req = urllib.request.Request(
        ...
        headers={"Authorization": f"Bearer {api_key}",
                 ...
                 # Groq's edge rejects the default "Python-urllib" UA with 403
                 "User-Agent":    "roadsafety-sim/2.0"},
    )
...
async def copilot_status():
    """Whether the Copilot is configured (key present) — without revealing the key."""
    return {"configured": bool(_groq_key()), "model": GROQ_MODEL}
...
async def copilot(payload: dict):
    ...
    key = _groq_key()
    if not key:
        raise HTTPException(503, "Copilot not configured on the server")
    ...
    try:
        text = await asyncio.to_thread(_groq_chat, messages, key)
